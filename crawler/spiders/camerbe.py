...
class CamerBe(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for articles in response.css(".card"):
    ...
    def parse1(self, response):
        article = response.css("article .card")
        print(response)
        url = response.url
        image = article.css(".card img::attr(src)").get()
        title = self.clean_string(article.css("h1.card-title::text").get())
        excerpt = article.css("div.info p::text").getall()[2]
