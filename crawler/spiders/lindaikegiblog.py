...
class LidaIkeji(scrapy.Spider):
    ...
    def parse1(self, response):
        ...
        url = response.url
        img = article.css("summary img::attr(src)").getall()[1]
        title = self.clean_string(article.css("h1.title::text").get())
        # date = self.clean_string(article.css("span.timestamp::text").get())
        excerpt = article.css("summary p::text").getall()[2]
        page = response.url.split("/")[2]
