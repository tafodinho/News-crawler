...
class TheTribuneOnlineNG(scrapy.Spider):
    name = "thetribuneonlineng"
    table = "nigerias"
    ...
    def parse(self, response):
        ...
        for articles in response.css("div.listing article"):
    ...
    def parse1(self, response):
        ...
        url = response.url
        img = article.css(".single-featured img::attr(src)").get()
        title = self.clean_string(article.css("h1 span.post-title::text").get())
